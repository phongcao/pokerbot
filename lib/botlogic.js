...
({
    ...
    default: Defs.Path.DEFAULT_DOCUMENT
}));
...
{
    var leaderboardCache = Game.LEADERBOARD_CACHE;
    ...
    { 
        ...
        usersDirs.forEach(function(userDir, index)
        {
            fs.readFile(path.join(usersRootDir, userDir, Defs.Path.PLAYER_INFO_FILE), 'utf8', function (err, data) 
            {
                if (!err)
                {
                    try 
                    {
                        ...
                        var userId = userDir.slice(0, 2) + ":" + userDir.slice(2, userDir.length);
...
[
    function (session) 
    {
        ...
        if (!fs.existsSync(session.userData.localDir))
        {
            fs.mkdirSync(session.userData.localDir);
...
[
    ...
    function (session, results) 
    {
        if (results.response && results.response.entity == Strings.YES) 
        {

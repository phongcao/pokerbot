...
function initPlayers(session)
{
    ...
    humanPlayerIndex = 0;
    players.push(new Player(session, false, Defs.Board.ROUND_BUYINS, HUMAN_CARD_X, HUMAN_CARD_Y, HUMAN_DEALER_CHIP_X, HUMAN_DEALER_CHIP_Y));

    // Init NPC
    for (var i = 1; i < totalPlayers; i++)
    {
        players.push(new Player(session, true, Defs.Board.ROUND_BUYINS, NPC_CARD_X, NPC_CARD_Y, NPC_DEALER_CHIP_X, NPC_DEALER_CHIP_Y));
    }
...
function processPlayerBettingOption(session, player, bettingOption)
{
    ...
    {
        player.log(session, communityCards, calculatePot(), players[1]);
    }
...
function stateBoard(session)
{
    ...
    else
    {
        ...
        renderBoardCanvas(session, zeroFundPlayersCount < (totalPlayers - 1), function (result)
        {
            switchState(session, Defs.GameState.STATE_PLAYERS_INPUT, true);
        });
    }
...
function statePlayersInput(session)
{
    ...
    else if (finishBetting || (zeroFundPlayersCount >= (totalPlayers - 1)))
    {
        roundIndex++;
        currentBet = 0;
...
function stateResult(session)
{
    ...
    session.userData.playerInfo.allin = players[humanPlayerIndex].allInCount;
    ...
    else
    {
        if (winnerIndices.length == 1)
        {
            ...
            {
                var pot = calculatePot();
                var winningAmount = pot - players[winnerIndices[humanPlayerIndex]].betAmount;
                var winEmoticons = "";

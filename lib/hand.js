...
{
    var length = Math.min(hand1.length, hand2.length);
    for (var i = 0; i < length; i++)
    {
        if (hand1[i].rank != hand2[i].rank)
        {
            return (hand1[i].rank < hand2[i].rank) ? -1 : 1;
        }

        if (hand1[i].highcard.value != hand2[i].highcard.value)
        {
            return (hand1[i].highcard.value < hand2[i].highcard.value) ? -1 : 1;
        }
    }
